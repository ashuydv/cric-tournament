...
const LeftHero = ({
    ...
    const buttonVariants: Record<string, string> = {
        ...
        outline: 'bg-transparent border-2 border-orange-500 hover:bg-orange-500/10 text-orange-500'
    }
    ...
    const getBackgroundStyle = () => {
        ...
        if (backgroundType === 'gradient') {
            return {
                background: backgroundSrc || 'linear-gradient(to right, #ff4e00, #ec9f05)'
            } as React.CSSProperties
        }
    ...
    const renderPrimaryContent = () => {
        ...
        const primaryWidthClass = layout !== 'default'
            ? (layout === 'left-content'
            ...
            return (
                ...
                >
                    ...
                    <motion.div className="space-y-3" variants={itemVariants}>
                        <h1 className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold ${textColor} leading-tight font-playfair`}>
                            {title}
                        </h1>

                        {/* Subtitle */}
                        {subtitle && (
                            <p className={`text-xl sm:text-2xl md:text-3xl font-semibold ${highlightColor}`}>
                                {subtitle}
                            </p>
                        )}
                        ...
                        {/* Description */}
                        {description && (
                            <p className={`max-w-[700px] text-gray-400 md:text-lg ${align === 'center' ? 'mx-auto' : ''} mt-4`}>
                                {description}
                            </p>
                        )}
                    </motion.div>
                    ...
                    {buttons && buttons.length > 0 && (
                        <motion.div
                            className={`flex flex-wrap gap-4 mt-8 ${align === 'center' ? 'justify-center' :
                        ...
                        >
                            {buttons.map((button, index) => (
                                <Link
                                    href={button.href}
                                    key={index}
                                    className={`px-6 py-3 rounded-lg font-medium transition-all duration-300 flex items-center gap-2 ${buttonVariants[button.variant]}`}
                                >
                                    {button.text}
        ...
        return (
            <div className={contentClassName} style={parallaxEnabled ? { transform: `translateY(${contentY}px)` } : {}}>
                {/* Logo */}
                {logoVisible && (
                    <div className="mb-4">
                        <div className="w-20 h-20 md:w-24 md:h-24 relative mx-auto">
    ...
    const renderSecondaryContent = () => {
        ...
        const secondaryContentElement = secondaryContent || (
            secondaryContentType === 'image' ? (
        ...
        // Safely access the split ratio classes with type checking
        const secondaryWidthClass = splitRatioClasses[layout === 'right-content'
            ? splitRatio
            : complementaryRatio[splitRatio]];
        ...
        return (
            <div
                className={className}
                style={parallaxEnabled ? { transform: `translateY(${contentY}px)` } : {}}
            >
                {secondaryContentElement}
            </div>
        );
    ...
    return (
        ...
        >
            ...
            {/* Overlay Layer */}
            {backgroundOverlay && (backgroundType === 'image' || backgroundType === 'video') && (
                <div
